...
async def load_items(items: list[dict], session: AsyncSession) -> int:
    ...
    for item in items:
        ...
        # Map short lab ID (e.g., "lab-01") to the DB record
        lab_id_map[item["lab"]] = lab_item
...
async def load_logs(
    ...
    for item in items_catalog:
        ...
        title = item["title"]
        item_title_map[(lab_id, task_id)] = title
    ...
    for log in logs:
        ...
        lab_id = log["lab"]
        task_id = log.get("task")  # None for lab-level logs
        item_title = item_title_map.get((lab_id, task_id))
        ...
        # 3. Check for existing interaction (idempotent upsert)
        external_id = log["id"]
